...
for((i=0; i<$len; i++)) {
    experimentAccession=`echo "$EXPERIMENTS_RESULTS" | jq .aaData["$i"].experimentAccession | tr -d '"'`
    experimentType=`echo "$EXPERIMENTS_RESULTS" | jq .aaData["$i"].experimentType | tr -d '"'`
...
#       echo ${GENE_DISTRIBUTION}
#       echo ${experimentAccession} "-" ${experimentType}
