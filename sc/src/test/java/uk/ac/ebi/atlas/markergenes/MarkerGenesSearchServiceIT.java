...
public class MarkerGenesSearchServiceIT {
    ...
    public void searchIdsOfGenesWithHighProbability() throws Exception {
        List<String> fiveMarkerGeneIds =
                jdbcTemplate.queryForList(
                        "SELECT DISTINCT(gene_id) FROM scxa_marker_genes WHERE marker_probability>? LIMIT 5",
    ...
    public void searchIdsOfGenesWithLowProbability() throws Exception {
        List<String> fiveNonMarkerGeneIds = jdbcTemplate.queryForList(
                "SELECT all_p.gene_id FROM scxa_marker_genes all_p LEFT JOIN " +
                "  (SELECT gene_id, marker_probability FROM scxa_marker_genes WHERE marker_probability>?) high_p ON all_p.gene_id=high_p.gene_id " +
