...
public class TSnePlotService {
    ...
    public Map<String, Set<TSnePoint>> fetchTSnePlotWithMetadata(String experimentAccession, int perplexity, String metadataCategory) {
        List<TSnePoint.Dto> pointDtos = tSnePlotServiceDao.fetchTSnePlotForPerplexity(experimentAccession, perplexity);

        // An alternative implementation would be to get the metadata for each cell in the tSnePlotServiceDao method and
        // create TSnePoint.Dto objects with metadata values. This would require separate requests to Solr for each cell ID.
