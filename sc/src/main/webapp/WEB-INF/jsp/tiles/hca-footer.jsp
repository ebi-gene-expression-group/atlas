<%@ page contentType="text/html;charset=UTF-8" %>

<footer id="local-footer" class="local-footer" role="local-footer">
    <div id="relationships" class="row">
        <div class="small-4 columns text-center padding-top-xlarge">
            <a href="https://www.ebi.ac.uk/" class="clear">
                <img src="${pageContext.request.contextPath}/resources/images/logos/embl_ebi_logo.png"
                     alt="EMBL-EBI logo" style="height: 4em">
            </a>
        </div>

        <div class="small-4 columns text-center padding-top-xlarge">
            <a href="https://www.sanger.ac.uk/" class="clear">
                <img src="${pageContext.request.contextPath}/resources/images/logos/sanger_institute.png"
                     alt="Wellcome Sanger Institute logo" style="height: 4em">
            </a>
        </div>
        <div class="small-4 columns text-center padding-top-xlarge">
            <a href="https://wellcome.ac.uk/" class="clear">
                <img src="${pageContext.request.contextPath}/resources/images/logos/wellcome_trust_logo_black.png"
                     alt="Wellcome Trust logo" style="height: 4em">
            </a>
        </div>
    </div>
</footer>