...
const BlueprintDisclaimer = () =>
  <div>
    ...
    <p>
      ...
      non-neoplastic diseases studied within the consortium. Talks, posters, and papers on all such analyses
    ...
    <p>
      Researchers who have used small amounts of Project data (&lt= one chromosome) may present methods
    ...
    <p>
      ...
      who contributed to the generation of the data is available from <a href={`http://www.blueprint-epigenome.eu`}>
      www.blueprint-epigenome.eu</a>. Funding for the project was provided by the European Union's Seventh
      Framework Programme (FP7/2007-2013) under grant agreement no 282510 – BLUEPRINT.
    </p>
  </div>

const FortLauderdaleDisclaimer = () =>
  <div>
    <h3>Fort Lauderdale Data Reuse Statement</h3>
    <p>
...
const PanCancerDisclaimer = () =>
  <div>
    <h3>PCAWG Publication and Embargo Policy</h3>
    <p>
      The PanCancer Analysis of Whole Genomes (PCAWG) project is committed to the principle of rapid distribution
      of primary data, alignments files, and secondary analysis data (such as mutation calls) to all members of
      the research community that have applied for (and have become approved for) data access. The primary data
      contributed to the project will fall under any publication/embargo policy of the contributing institute or
      consortia. The community resource for the PCAWG project comprises newly generated files produced by this
      collaboration (unified alignments, mutation calls and any secondary analysis data). As is the case for
      similar "community resource projects", users of data generated and distributed by the PCAWG project are
      asked to respect the desire of the PCAWG Consortium to publish reports on the generation and analysis of
      their data. Hence, scientific works with a primary focus on pan-cancer (across entity) analysis that made
      use of the WGS pan cancer community resource will be under publication embargo until the WGS pan-cancer
      consortium publishes its main paper (anticipated end of 2017). Methodology papers may be published prior to
      this embargo, with agreement from the full scientific working group.
    </p>
    <p>
      Please write to <a href={`mailto:jennifer.jennings@oicr.on.ca`}>Jennifer Jennings
      &lt;jennifer.jennings@oicr.on.ca&gt;</a> for further
      information.
    </p>
  </div>

export default {
  blueprint: BlueprintDisclaimer,
  lauderdale: FortLauderdaleDisclaimer,
  pcawg: PanCancerDisclaimer
}
