...
curl -o /dev/null -X GET -s -w %{http_code}:%{time_connect}:%{time_starttransfer}:%{time_total} "${ATLAS_ROOT_URL}/widgets/heatmap/referenceExperiment?geneQuery=REACT_111102&geneSetMatch=true" | awk -F":" '{print "/widgets/heatmap/referenceExperiment?geneQuery=REACT_111102&geneSetMatch=true                              "$1"          "$2"           "$3"                 "$4}'
curl -o /dev/null -X GET -s -w %{http_code}:%{time_connect}:%{time_starttransfer}:%{time_total} "${ATLAS_ROOT_URL}/widgets/heatmap/referenceExperiment?geneQuery=ENSG00000187003+ENSG00000185264" | awk -F":" '{print "/widgets/heatmap/referenceExperiment?geneQuery=ENSG00000187003+ENSG00000185264      "$1"          "$2"           "$3"                 "$4}'
