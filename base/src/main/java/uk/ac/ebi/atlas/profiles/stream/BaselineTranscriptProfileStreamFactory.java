 ...
 */
@Named
public class BaselineTranscriptProfileStreamFactory extends ProfileStreamKryoLayer<AssayGroup, BaselineExpressionPerBiologicalReplicate, BaselineExperiment, BaselineRequestContext<ExpressionUnit.Absolute.Rna>, BaselineExpressionPerReplicateProfile> {
    ...
    static class Impl extends CreatesProfilesFromTsvFiles<AssayGroup, BaselineExpressionPerBiologicalReplicate,
            BaselineExperiment, BaselineRequestContext<ExpressionUnit.Absolute.Rna>, BaselineExpressionPerReplicateProfile> {
        protected Impl(DataFileHub dataFileHub) {
            super(dataFileHub);
        }

        @Override
        protected Predicate<BaselineExpressionPerBiologicalReplicate> filterExpressions(BaselineExperiment experiment, BaselineRequestContext<ExpressionUnit.Absolute.Rna> options) {
        /*
            ...
            return header -> {
                ...
                for (int i = 3; i < header.length; i++) {
                    ...
                    experiment.getDataColumnDescriptors().stream().flatMap(assayGroup ->
                            assayGroup.biologicalReplicatesForThisDataColumn()
                                    .stream()
                ...
                return line -> {
                    ...
                    Multimap<AssayGroup, Pair<BiologicalReplicate, BaselineExpression>> expressionsPerAssayGroup = LinkedListMultimap.create(experiment.getDataColumnDescriptors().size());
                    ...
                    BaselineExpressionPerReplicateProfile result = new BaselineExpressionPerReplicateProfile(transcriptId, transcriptId);
                    for (Map.Entry<AssayGroup, Collection<Pair<BiologicalReplicate, BaselineExpression>>> e : expressionsPerAssayGroup.asMap().entrySet()) {
                        //If needed add the call to expression filter here.
                        result.add(e.getKey(), new BaselineExpressionPerBiologicalReplicate(e.getValue().stream().collect(Collectors.toMap(
                                Pair::getLeft,
                                Pair::getRight
                        ))));
                    }
        ...
        @Override
        protected Collection<ObjectInputStream<String[]>> getDataFiles(BaselineExperiment experiment, BaselineRequestContext<ExpressionUnit.Absolute.Rna> options) {
