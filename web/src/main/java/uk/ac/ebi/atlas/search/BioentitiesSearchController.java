...
//            Optional<String> geneIdRedirectString = getGeneIdRedirectString(requestParameters.getGeneQuery(), selectedSpecies);
//            if (geneIdRedirectString.isPresent()) {
//                redirectAttributes.addFlashAttribute("searchDescription", requestParameters.getGeneQuery().as1DNF());
...
//
//        Optional<Set<String>> geneIdsFromGeneQuery = solrQueryService.expandGeneQueryIntoGeneIds(geneQueryString, selectedSpecies.toLowerCase());
//
...
//
//        if (geneQuery.size() == 1 && GeneSetUtil.isGeneSetCategoryOrMatchesGeneSetAccession(geneQuery)) {
//            return Optional.of("redirect:/genesets/" + geneQuery.terms().iterator().next().value());
//        }
//
...
//           species = "";
//        }
//        Optional<Set<String>> geneIdsOrSets = solrQueryService.expandGeneQueryIntoGeneIds(geneQuery.asString().trim(), species);
//
