    ...
    public String showGeneQueryResultPage(@Valid GeneQuerySearchRequestParameters requestParameters, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (requestParameters.hasSemanticQuery() && !requestParameters.hasCondition()) {
            if (requestParameters.getSemanticQuery().size() == 1) {
                ...
                return multiTermGeneQuery(requestParameters.getSemanticQuery(), selectedSpecies, requestParameters.isExactMatch(), model, redirectAttributes);
            }
...
//        return "bioEntities";
    ...
    private String multiTermGeneQuery(SemanticQuery geneQuery2, String species, boolean isExactMatch, Model model, RedirectAttributes redirectAttributes) {
        return "";
    }
