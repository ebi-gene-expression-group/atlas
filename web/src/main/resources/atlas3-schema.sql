...
CREATE INDEX RSQ_BSLN_EXPRESSIONS_NEW_IDX ON RNASEQ_BSLN_EXPRESSIONS_NEW (IDENTIFIER, ISACTIVE, EXPERIMENT, ASSAYGROUPID) TABLESPACE ATLASPRD3_INDX;

-- Populate the new tables, to make them ready for testing
insert into MICROARRAY_DIFF_ANALYTICS_NEW select * from MICROARRAY_DIFF_ANALYTICS PARTITION (DIFF_ANALYTICS_ACTIVE);
insert into RNASEQ_DIFF_ANALYTICS_NEW select * from RNASEQ_DIFF_ANALYTICS PARTITION (DIFF_ANALYTICS_ACTIVE);
insert into RNASEQ_BSLN_EXPRESSIONS_NEW select * from RNASEQ_BSLN_EXPRESSIONS PARTITION (BSLN_EXPRESSIONS_ACTIVE);
exec dbms_mview.refresh( 'VW_DIFFANALYTICS_DUMP_NEW', 'C' );
exec dbms_mview.refresh( 'VW_DIFFANALYTICS_NEW', 'C' );
