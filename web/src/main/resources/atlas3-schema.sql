...
------------ rpetry: Post partitioning-elimination tables - testing start -------------

-- Microarray differential analytics
DROP TABLE MICROARRAY_DIFF_ANALYTICS_NEW;
CREATE TABLE MICROARRAY_DIFF_ANALYTICS_NEW (
    DESIGNELEMENT VARCHAR2(255) NOT NULL
  , EXPERIMENT VARCHAR2(255) NOT NULL
  , ARRAYDESIGN  VARCHAR(255) NOT NULL
  , CONTRASTID VARCHAR2(255) NOT NULL
...
CREATE INDEX PK_MA_DIFF_ANALYTICS_NEW ON MICROARRAY_DIFF_ANALYTICS_NEW (DESIGNELEMENT, EXPERIMENT, CONTRASTID, PVAL) TABLESPACE ATLASPRD3_INDX;
ALTER TABLE MICROARRAY_DIFF_ANALYTICS_NEW
  ADD CONSTRAINT PK_MA_DIFF_ANALYTICS_NEW
  PRIMARY KEY (DESIGNELEMENT, EXPERIMENT, CONTRASTID, PVAL) USING INDEX PK_MA_DIFF_ANALYTICS_NEW
ENABLE;

CREATE INDEX MA_DIFF_ANALYTICS_NEW_IDX ON MICROARRAY_DIFF_ANALYTICS_NEW (EXPERIMENT, CONTRASTID) TABLESPACE ATLASPRD3_INDX;


-- RNA-seq differential analytics
DROP TABLE RNASEQ_DIFF_ANALYTICS_NEW;
CREATE TABLE RNASEQ_DIFF_ANALYTICS_NEW (
    IDENTIFIER VARCHAR2(255) NOT NULL
  , EXPERIMENT VARCHAR2(255) NOT NULL
  , CONTRASTID VARCHAR2(255) NOT NULL
  , PVAL FLOAT(125) NOT NULL
...
CREATE INDEX PK_RNASEQ_DIFF_ANALYTICS_NEW ON RNASEQ_DIFF_ANALYTICS_NEW (IDENTIFIER, EXPERIMENT, CONTRASTID, PVAL) TABLESPACE ATLASPRD3_INDX;
ALTER TABLE RNASEQ_DIFF_ANALYTICS_NEW
  ADD CONSTRAINT PK_RNASEQ_DIFF_ANALYTICS_NEW
  PRIMARY KEY (IDENTIFIER, EXPERIMENT, CONTRASTID, PVAL) USING INDEX PK_RNASEQ_DIFF_ANALYTICS_NEW
ENABLE;

CREATE INDEX RNASEQ_DIFF_ANALYTICS_NEW_IDX ON RNASEQ_DIFF_ANALYTICS_NEW (EXPERIMENT, CONTRASTID) TABLESPACE ATLASPRD3_INDX;

-- Differential analitics across all types of experiments, with additional gene name and organism information, restricted to FDR<0.05 and log2fold>=1
-- Used by the UI
DROP MATERIALIZED VIEW VW_DIFFANALYTICS_NEW;
CREATE MATERIALIZED VIEW VW_DIFFANALYTICS_NEW
NOLOGGING PARALLEL 16
BUILD DEFERRED
REFRESH COMPLETE ON DEMAND
ENABLE QUERY REWRITE AS
...
select dem.IDENTIFIER, bn.NAME AS NAME, o.NAME AS ORGANISM, mda.EXPERIMENT, mda.CONTRASTID, mda.PVAL, mda.LOG2FOLD, mda.TSTAT
,rank() over(partition by mda.EXPERIMENT, mda.CONTRASTID, dem.IDENTIFIER order by(abs(mda.LOG2FOLD)) desc) as lfrank
from MICROARRAY_DIFF_ANALYTICS_NEW mda
join DESIGNELEMENT_MAPPING dem on mda.designelement=dem.designelement and mda.arraydesign = dem.arraydesign
join BIOENTITY_NAME bn on dem.identifier=bn.identifier
join BIOENTITY_ORGANISM o on bn.organismid = o.organismid
where abs(mda.LOG2FOLD) >= 1
...
union all
select rda.IDENTIFIER, bn.NAME AS NAME, o.name AS ORGANISM, rda.EXPERIMENT, rda.CONTRASTID, rda.PVAL, rda.LOG2FOLD, null
from RNASEQ_DIFF_ANALYTICS_NEW rda
join BIOENTITY_NAME bn on rda.IDENTIFIER=bn.identifier
join BIOENTITY_ORGANISM o on bn.organismid = o.organismid
join EXPERIMENT_ORGANISM eo on o.name = eo.bioentity_organism and eo.experiment = rda.experiment
where abs(rda.LOG2FOLD) >= 1
...
exec dbms_mview.refresh( 'VW_DIFFANALYTICS_NEW', 'C' );

-- Differential analitics across all types of experiments, with additional gene name and organism information, restricted to FDR<0.05 only
-- Used to generate a data dump at Atlas release time
DROP MATERIALIZED VIEW VW_DIFFANALYTICS_DUMP_NEW;
CREATE MATERIALIZED VIEW VW_DIFFANALYTICS_DUMP_NEW
NOLOGGING PARALLEL 16
BUILD DEFERRED
REFRESH COMPLETE ON DEMAND
ENABLE QUERY REWRITE AS
...
exec dbms_mview.refresh( 'VW_DIFFANALYTICS_DUMP_NEW', 'C' );

-- RNA-seq baseline analytics
DROP TABLE RNASEQ_BSLN_EXPRESSIONS_NEW;
CREATE TABLE RNASEQ_BSLN_EXPRESSIONS_NEW (
    IDENTIFIER VARCHAR2(255) NOT NULL
  , EXPERIMENT VARCHAR2(255) NOT NULL
  , ASSAYGROUPID VARCHAR2(255) NOT NULL
  , EXPRESSION FLOAT(125) NOT NULL
...
CREATE INDEX PK_RSQ_BSLN_ANALYTICS_NEW ON RNASEQ_BSLN_EXPRESSIONS_NEW (IDENTIFIER, EXPERIMENT, ASSAYGROUPID, EXPRESSION) TABLESPACE ATLASPRD3_INDX;
ALTER TABLE RNASEQ_BSLN_EXPRESSIONS_NEW
  ADD CONSTRAINT PK_RSQ_BSLN_EXPRESSIONS_NEW
  PRIMARY KEY (IDENTIFIER, EXPERIMENT, ASSAYGROUPID, EXPRESSION) USING INDEX PK_RSQ_BSLN_ANALYTICS_NEW
ENABLE;

CREATE INDEX RSQ_BSLN_EXPRESSIONS_NEW_IDX ON RNASEQ_BSLN_EXPRESSIONS_NEW (IDENTIFIER, EXPERIMENT, ASSAYGROUPID) TABLESPACE ATLASPRD3_INDX;

-- Populate the new tables, to make them ready for testing
insert into MICROARRAY_DIFF_ANALYTICS_NEW select DESIGNELEMENT, EXPERIMENT, ARRAYDESIGN, CONTRASTID, PVAL, LOG2FOLD, TSTAT from MICROARRAY_DIFF_ANALYTICS PARTITION (DIFF_ANALYTICS_ACTIVE);
insert into RNASEQ_DIFF_ANALYTICS_NEW select IDENTIFIER, EXPERIMENT, CONTRASTID, PVAL, LOG2FOLD from RNASEQ_DIFF_ANALYTICS PARTITION (DIFF_ANALYTICS_ACTIVE);
insert into RNASEQ_BSLN_EXPRESSIONS_NEW select IDENTIFIER, EXPERIMENT, ASSAYGROUPID, EXPRESSION from RNASEQ_BSLN_EXPRESSIONS PARTITION (BSLN_EXPRESSIONS_ACTIVE);

exec dbms_mview.refresh( 'VW_DIFFANALYTICS_DUMP_NEW', 'C' );
exec dbms_mview.refresh( 'VW_DIFFANALYTICS_NEW', 'C' );

------------ rpetry: Post partitioning-elimination tables - testing end -------------
